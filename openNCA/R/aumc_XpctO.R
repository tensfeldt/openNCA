...
#' aumc_XpctO(mod)  
#' #Please specify for which subject you want to get the AUMC_XPCTO for!
#' 
#' aumc_XpctO(mod, sid = "all") 
#' #  SID              AUC     METHOD
#' #   30 4.94993257703106 Linear-Log
#' #   31                1 Linear-Log
#' #   32 5.15416479501756 Linear-Log
#' 
#' aumc_XpctO(mod, sid = 32)  
#' #  SID              AUC     METHOD
#' #   32 5.15416479501756 Linear-Log
#' 
#' aumc_XpctO(mod, sid = 10)  
#' #Invaild subject ID! Subject ID not found in the data provided!
#' 
#' aumc_XpctO(mod, sid = 31, method = 5)
...
#' \figure{rudraya.png} 
#' @export
aumc_XpctO <- function(conc = NULL, time = NULL, method = 1){
  ...
  if(sum(conc, na.rm=T) == 0){
    ...
    if(is.na(aumc_info) || aumc_info == 0 || is.na(aumclast)){
      aumc_xpcto <- NA
      return(aumc_xpcto)
    } else {
      aumc_xpcto <- ((aumc_info - aumclast)/aumc_info)*100
      return(aumc_xpcto)
    }
  }
}