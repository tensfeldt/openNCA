...
#'
#' @section Note:
#' The inputs 'kelflag', 'auclast', 'aucinfp' and 'auclast' are optional. If 'aucinfp' input is not provided then it will generate the AUCINFP using the concentration and time data provided. \cr
...
#'
#' #auc_XpctP()
#' #Error in auc_XpctP: 'conc' and 'time' vectors are NULL
#'
...
#' }
#' @export
auc_XpctP <- function(conc = NULL, time = NULL, method = 1, kelflag = NULL, aucflag = NULL, auc_infp = NULL, auclast = NULL, interpolate = NULL, model = NULL, dosing_type = NULL, told = NULL, orig_conc = NULL, orig_time = NULL){
  ...
  if(sum(conc, na.rm=T) == 0){
    ...
    if(is.null(auc_infp)){
      auc_infp <- auc_inf_p(conc = conc, time = time, method = method, kelflag = kelflag, aucflag = aucflag, interpolate = interpolate, model = model, dosing_type = dosing_type, told = told, orig_conc = orig_conc, orig_time = orig_time)
    }
    if(is.null(auclast)){
