...
#' @param auc_info The area under the concentration versus time curve from zero time to infinity (Observed) (numeric value)
...
auc_XpctO <- function(conc = NULL, time = NULL, method = 1, kelflag = NULL, aucflag = NULL, auc_info = NULL, auclast = NULL) {
  ...
  if(sum(conc, na.rm=T) == 0){
    ...
    if(isTRUE(is.na(auc_info) || auc_info == 0 || is.na(auclast))){
      auc_xpcto <- NA
      return(auc_xpcto)
    } else {
