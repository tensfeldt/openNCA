#' Percentage of AUMCINFP obtained by forward extrapolation
#'
#' Percentage of AUMCINFP obtained by forward extrapolation.\cr
#'
#' @details
#' \strong{Equation}
#' \enumerate{
...
#' \strong{aumc_inf_p}: Refer to \code{\link{aumc_inf_p}} for more details
...
#' \strong{Value} \cr
#' \itemize{
#'  \item AUMCXPCTP: percentage of AUMCINFP obtained
#' }
#'
#' @examples
#' ##########
...
#'
#' #aumc_XpctP()
#' #Error in aumc_XpctP: 'conc' and 'time' vectors are NULL
#'
#' conc_vector <- c(2.89, 2.49, 2.47, 2.38, 2.32, 2.28)
#' time_vector <- c(0, 1, 2, 3, 4, 5)
#'
#' aumc_XpctP(conc = conc_vector, time = time_vector)
#' #98.175
#'
#' ############
#' ## Data 2 ##
...
aumc_XpctP <- function(conc = NULL, time = NULL, method = 1, kelflag = NULL, aucflag = NULL, spanratio = NULL, kel = NULL, aumcinfp = NULL, aumclast = NULL){
  if(is.null(conc) && is.null(time)){
    ...
    stop("Error in aumc_XpctP: 'conc' vector is NULL")
  } else if(is.null(time)) {
    stop("Error in aumc_XpctP: 'time' vectors is NULL")
  ...
  if(!(is.numeric(conc) && is.vector(conc)) ){
    stop("Error in aumc_XpctP: 'conc' is not a numeric vector")
  }
  if(!(is.numeric(time) && is.vector(time)) ){
    stop("Error in aumc_XpctP: 'time' is not a numeric vector")
  }
  if(length(time) != length(conc)){
    stop("Error in aumc_XpctP: length of 'time' and 'conc' vectors are not equal")
  }
