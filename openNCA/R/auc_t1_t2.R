#' Area under the concentration versus time cruve from time T1 until time T2
#'
...
#' #Error in auc_t1_t2(conc, time, 1) : 
#' #  Error in auc_t1_t2: 't2' value is not present in 'time'
#' 
...
#' #Error in auc_t1_t2(conc, time, t1 = 1, t2 = 1) : 
...
#' }
#' @export
auc_t1_t2 <- function(conc = NULL, time = NULL, t1 = NULL, t2 = NULL, method = 1){
  ...
  if(is.null(t1) || !(t1 %in% time)){
    stop("Error in auc_t1_t2: 't1' value is not present in 'time'")
  }
  if(is.null(t2) || !(t2 %in% time)){
    stop("Error in auc_t1_t2: 't2' value is not present in 'time'")
  }
  if(t1 >= t2){
