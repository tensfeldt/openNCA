...
###        df <- data.frame(SDEID=rep(i, length(col_names)), col_names=col_names, row_data=c(row_data))
  ...
  if("FLGEMESIS" %in% names(map_data) && ("TMAX" %in% parameter_list)){
    computation_df$FLGACCEPTTMAX <- ifelse(computation_df$FLGEMESIS == 1 & computation_df$TMAX1 < 2 * median(computation_df$TMAX1), TRUE, FALSE)
  }
...
### 2019-10-01/TGT/ Remove display_list and display_parameters code
  if(F) {    
...
###    if("AUCT1_T2" %in% display_list && auc_pair_check) {
    if(disp_required[["AUCT1_T2"]] && auc_pair_check) {
        display_parameters <- c(display_parameters, rep(paste0("AUC_", map_data[,rep(paste0("AUC.", 1:auc_par_len, ".T1"))], "_", map_data[,rep(paste0("AUC.", 1:auc_par_len, ".T2"))])))
    }
...
###    if("CLO" %in% display_list) {
    if(disp_required[["CLFO"]]) {
      display_parameters <- c(display_parameters, "CLFO")
    }
###    if("CLFOW" %in% display_list) {
    if(disp_required[["CLFOW"]]) {
      display_parameters <- c(display_parameters, "CLFOW")
    }
###    if("CLFP" %in% display_list) {
    if(disp_required[["CLFP"]]) {
      display_parameters <- c(display_parameters, "CLFP")
    }
###    if("CLFPW" %in% display_list) {
    if(disp_required[["CLFPW"]]) {
      display_parameters <- c(display_parameters, "CLFPW")
    }
###    if("VZFO" %in% display_list) {
    if(disp_required[["VZFO"]]) {
      display_parameters <- c(display_parameters, "VZFO")
    }
###    if("VZFOW" %in% display_list) {
    if(disp_required[["VZFOW"]]) {
      display_parameters <- c(display_parameters, "VZFOW")
    }
###    if("VZFP" %in% display_list) {
    if(disp_required[["VZFP"]]) {
      display_parameters <- c(display_parameters, "VZFP")
    }
###    if("VZFPW" %in% display_list) {
    if(disp_required[["VZFPW"]]) {
      display_parameters <- c(display_parameters, "VZFPW")
    }
...
###    ctk <- grep("^CONC(TIME)*?([0-9]+?)$", names(computation_df))
###    ctn <- names(computation_df)[ctk]

###    k1 <- c(names(computation_df)[names(computation_df) %in% display_parameters], names(x))
###    k1 <- c(k1, ctn)
###    cat("k1: ", k1, "\n")

###    k2 <- c(names(computation_df)[names(computation_df) %in% parameter_list], names(x))
###    k2 <- c(k2, ctn)
###    cat("k2: ", k2, "\n")
###    computation_df <- computation_df[,k1]
