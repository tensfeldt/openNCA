...
auc_inf_pc <- function(kel = NULL, aucinfp = NULL, c0 = NULL){
  if(is.null(kel) && is.null(aucinfp) && is.null(c0)){
    stop("Error in auc_inf_pc: 'kel', 'aucinfp' and 'c0' vectors are NULL")
  } else if(is.null(kel) && is.null(aucinfp)){
    stop("Error in auc_inf_pc: 'kel' and 'aucinfp' vectors are NULL")
  } else if(is.null(kel) && is.null(c0)){
    stop("Error in auc_inf_pc: 'kel' and 'c0' vectors are NULL")
  } else if(is.null(aucinfp) && is.null(c0)){
    stop("Error in auc_inf_pc: 'aucinfp' and 'c0' vectors are NULL")
  } else if(is.null(kel)) {
    stop("Error in auc_inf_pc: 'kel' vector is NULL")
  } else if(is.null(aucinfp)) {
    stop("Error in auc_inf_pc: 'aucinfp' vectors is NULL")
  } else if(is.null(c0)) {
