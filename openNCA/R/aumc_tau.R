...
#' }
#' @export
aumc_tau <- function(conc = NULL, time = NULL, method = 1, exflag = NULL, told = NULL, tau = NULL, curr_di = NULL, max_di = NULL, t_max = NULL, orig_conc = NULL, orig_time = NULL, last_crit_factor = NULL, kel = NULL, aumclast = NULL, lasttime = NULL, orgtime = "nominal", nom_time = NULL, ctoldest = NULL, flgacceptkel = TRUE){
  ...
  if(!(is.numeric(curr_di) && is.vector(curr_di)) && !is.na(curr_di)){
    stop("Error in aumc_tau: 'curr_di' is not a numeric vector")
  }
  if(!(is.numeric(max_di) && is.vector(max_di)) && !is.na(max_di)){
    stop("Error in aumc_tau: 'max_di' is not a numeric vector")
  }
...
###  cat('aumc_tau.R: told: ', told, ' tau: ', tau, ' curr_di: ', curr_di, ' max_di: ', max_di, ' time: ', time, ' nomtime: ', nom_time,' conc: ', conc, ' method: ', method, ' exflag: ', exflag, ' flgacceptkel: ', flgacceptkel, ' t_max: ', t_max, ' orig_conc: ', orig_conc, ' orig_time: ', orig_time, '\n')
...
###          cat('aumctau: ', aumctau, '\n')
          ...
          if(isTRUE(time_min_range <= lasttime) && !is.null(aumclast)){
###          cat('aumclast: ', aumclast, '\n')
            aumctau <- aumclast
          } else {
            if(method == 1){
              aumctau <- aumc_lin_log(conc = conc, time = time, exflag = exflag, t_max = tmp_tmax)
            } else if(method == 2){
              aumctau <- aumc_lin(conc = conc, time = time, exflag = exflag)
            } else if(method == 3){
              aumctau <- aumc_log(conc = conc, time = time, exflag = exflag)
            } else if(method == 4){
              aumctau <- aumc_lin_up_log_down(conc = conc, time = time, exflag = exflag)
            }
          }
        }
      }
