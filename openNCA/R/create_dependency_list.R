...
create_dependency_list <- function() {
    ...
    dependency_list[["AUCT"]] <- list(callfun=c("auc_t1_t2"), regex="^AUC(T|[0-9]+?)$", unit_class=c(uclass_auc), valid_models=c(m1, m2, m3), display_list_models=c(), predecessors=c("TMAX"))
    dependency_list[["AUCT1_T2"]] <- list(callfun=c("auc_t1_t2"), regex="^AUC(T1|[0-9]+?)_(T2|[0-9]+?)$", unit_class=c(uclass_auc), valid_models=c(m1, m2, m3), display_list_models=c(), predecessors=c("TMAX", "AUCT"))
