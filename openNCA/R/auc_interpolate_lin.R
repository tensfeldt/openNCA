...
#' }
#' @export
auc_interpolate_lin <- function(conc = NULL, time = NULL){
  if(is.null(conc) && is.null(time)){
    stop("Error in auc_interpolate_lin: 'conc' and 'time' vectors are NULL")
  } else if(is.null(conc)) {
    stop("Error in auc_interpolate_lin: 'conc' vector is NULL")
  } else if(is.null(time)) {
    stop("Error in auc_interpolate_lin: 'time' vectors is NULL")
  }
  
  if(!(is.numeric(conc) && is.vector(conc)) ){
    stop("Error in auc_interpolate_lin: 'conc' is not a numeric vector")
  }
  if(!(is.numeric(time) && is.vector(time)) ){
    stop("Error in auc_interpolate_lin: 'time' is not a numeric vector")
  }
  if(length(time) != length(conc)){
    stop("Error in auc_interpolate_lin: length of 'time' and 'conc' vectors are not equal")
  }
  
  for(i in 2:(length(conc)-1)){
   if(is.na(conc[i])){
     tmp_t <- abs(((time[i] - time[i-1])/(time[i+1] - time[i-1])))
     conc[i] <- (conc[i-1] + (tmp_t * (conc[i+1] - conc[i-1])))
   } 
  }
  return(conc)
}