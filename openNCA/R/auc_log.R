...
##
      est_tmp <- estimate_missing_concentration(conc = conc, time = time, interpolate = interpolate, extrapolate = extrapolate, auc_method = "LOG", model = model, dosing_type = dosing_type, told = told, kel = kel, orig_conc = orig_conc, orig_time = orig_time)
      conc <- est_tmp[[1]]
      tmp <- data.frame(time, conc)
    }
    if(!is.na(t_max)){
