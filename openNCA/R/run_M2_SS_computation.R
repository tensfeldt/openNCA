...
#' }
#' @export
run_M2_SS_computation <- function(data = NULL, map = NULL, method = 1, model = "M2", parameter = "SS", return = list()){
  if(is.null(data)){
  ...
  if(tolower(return) == 'all' ||  (typeof(return) == 'list' && length(return) == 0)){
    ...
    for(i in 1:length(unique(data_data[,map_data$SDEID]))){
      ...
      computation_df[i,] <- c(unique(data_data[,map_data$SDEID])[i], c_0, unlist(v_0), unlist(c_max), unlist(c_min), unlist(c_last),
