...
#' @param model_regex The regular expression that identifies the model and dosing type (default: "^M2(SS)*?$")
...
#' \strong{Dataframe} \cr
#' \itemize{
#'  \item M2SS_Parameters: Calculated default/specified parameters
#' }
#' OR \cr
#' \strong{List} \cr
#' \itemize{
...
#' }
#' @export
run_M2_SS_computation <- function(data = NULL, map = NULL, method = 1, model_regex = "^M2(SS)*?$", parameter_list = list(), return_list = list(), raw_results_debug = FALSE, optimize_kel_debug = FALSE, ...){
  ...
  ###  disp_required[[i]] <- pr
  ###}
  all_parameters <- model_parameters("m2ss")
  ...
  ###if((disp_required[["AUCT"]] || disp_required[["AUCTDN"]]) && comp_required[["C0"]]) {
  if(disp_required[["AUCT"]] || disp_required[["AUCTDN"]]) {
    auc_len <- auc_len + 1 
    remove_extra_AUC <- TRUE
  } else {
  ...
  if(isTRUE(secondary)){
    ...
    if(disp_required[["MRAUCTAUi"]]){
      col_names <- c(col_names, rep(paste0("MRAUCTAU",1:di_col)))
      regular_int_type <- c(regular_int_type, rep(paste0("MRAUCTAU",1:di_col)))
    } 
    if(disp_required[["MRCMAX"]]) {
      col_names <- c(col_names, "MRCMAX")
      regular_int_type <- c(regular_int_type, "MRCMAX")
    }
    if(disp_required[["MRCMAXi"]]){
      col_names <- c(col_names, rep(paste0("MRCMAX",1:di_col)))
      regular_int_type <- c(regular_int_type, rep(paste0("MRCMAX",1:di_col)))
    } 
  }
      ...
      if(isTRUE(nrow(tmp_df) > 0 & all(tmp_df[,map_data$TIME][!is.na(tmp_df[,map_data$TIME])] >= 0)) & isTRUE(time_check) & isTRUE(conc_check)){
        ...
        for(d in 1:di_col){
          ...
          if(!isTRUE(ctold_exists) && !is.na(tmp_told)){
            ...
            tmp_nom_time_di <- tmp_di_df[,map_data$NOMTIME]
            
            est_tmp <- get_told_concentration(conc = tmp_conc_di, time = tmp_time_di, nom_time = tmp_nom_time_di, interpolate = TRUE, extrapolate = TRUE, auc_method = "LIN", model = "M2", dosing_type = "SS", dosing_interval = d, told = tmp_told, prev_told = tmp_prev_told, prev_tau = tmp_prev_tau, last_crit_factor = last_crit_factor, orig_conc = orig_conc, orig_time = orig_time, orgtime = tolower(map_data$ORGTIME))
          ...
          if((d+1) <= di_col){
            nxt_tmp_di_df  <- tmp_df[tmp_df[c(paste0("DI", d+1, "F"))] == 1,]
            nxt_tmp_di_df <- nxt_tmp_di_df[order(nxt_tmp_di_df[,map_data$TIME]),]
            nxt_tmp_conc_di <- nxt_tmp_di_df[,map_data$CONC]
            nxt_tmp_time_di <- nxt_tmp_di_df[,map_data$TIME]
            nxt_tmp_nom_time_di <- nxt_tmp_di_df[,map_data$NOMTIME]
            nxt_tmp_told <- as.numeric(nxt_tmp_di_df[, as.character(map_data[c(paste0("TOLD",d+1))])][1])
            nxt_tmp_tau <- as.numeric(nxt_tmp_di_df[, as.character(map_data[c(paste0("TAU",d+1))])][1])
            
            nxt_ctold_exists <- FALSE 
            if(nxt_tmp_told %in% nxt_tmp_di_df[,map_data$TIME]){
              idx <- which(nxt_tmp_di_df[,map_data$TIME] == nxt_tmp_told)
              nxt_tmp_ctold <- nxt_tmp_di_df[,map_data$CONC][length(idx)]
              if(!is.na(nxt_tmp_ctold)){
                nxt_ctold_exists <- TRUE
              }
            }
            if(!isTRUE(nxt_ctold_exists)){
              est_tmp <- get_told_concentration(conc = nxt_tmp_conc_di, time = nxt_tmp_time_di, nom_time = nxt_tmp_nom_time_di, interpolate = TRUE, extrapolate = TRUE, auc_method = "LIN", model = "M2", dosing_type = "SS", dosing_interval = d+1, told = nxt_tmp_told, prev_told = tmp_told, prev_tau = tmp_tau, last_crit_factor = last_crit_factor, orig_conc = orig_conc, orig_time = orig_time, orgtime = tolower(map_data$ORGTIME))
              nxt_tmp_conc_di <- est_tmp[[1]]
              nxt_tmp_time_di <- est_tmp[[2]]
              nxt_tmp_nom_time_di <- est_tmp[[3]]
              nxt_ctold_est <- est_tmp[[4]]
            } else {
              nxt_ctold_est <- NA
            }
          } else {
        ...
        for(d in 1:di_col){
          ...
          ###}
          dose_inf[[d]] <- dof(tmp_di_df, map_data, dof_name = paste0("DOF",d))
          ...
          ###}
          ###if(comp_required[["AUCINFPi"]]) {
            aucinfpi[[d]] <- auc_inf_p(conc = tmp_conc_di, time = tmp_di_df[,map_data$TIME], method = method, kelflag = kel_flag, aucflag = auc_flag, auclast = auclasti[[d]], t_last = t_lasti[[d]], kel = kel_v)
          ###}
          ###if(comp_required[["AUCINFPCi"]]){
          ...
          if(exists("flag_df")){
            ...
            for(f in 1:length(flag_df$VAR)){
              flag_param <- flag_df$VAR[f]
              if(toupper(flag_param) == "KEL"){
                if(isTRUE(eval(parse(text=paste(kel_v[["KEL"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
                } else {
              ...
              if(toupper(flag_param) == "KELCO"){
                if(isTRUE(eval(parse(text=paste(kel_v[["KELC0"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "KELTMLO"){
                if(isTRUE(eval(parse(text=paste(kel_v[["KELTMLO"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "KELTMHI"){
                if(isTRUE(eval(parse(text=paste(kel_v[["KELTMHI"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "KELNOPT"){
                if(isTRUE(eval(parse(text=paste(kel_v[["KELNOPT"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "KELR"){
                if(isTRUE(eval(parse(text=paste(kelr_v[["KELR"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "KELRSQ"){
                if(isTRUE(eval(parse(text=paste(kelr_v[["KELRSQ"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "KELRSQA"){
                if(isTRUE(eval(parse(text=paste(kelr_v[["KELRSQA"]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "AUCXPCTO"){
                if(isTRUE(eval(parse(text=paste(aucxpcto, flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "AUCXPCTOi"){
                if(isTRUE(eval(parse(text=paste(aucxpctoi[[d]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "AUCXPCTP"){
                if(isTRUE(eval(parse(text=paste(aucxpctp, flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
              ...
              if(toupper(flag_param) == "AUCXPCTPi"){
                if(isTRUE(eval(parse(text=paste(aucxpctpi[[d]], flag_df$OPR[f], flag_df$CRIT[f]))))){
                  if(isTRUE(tmp_flag_kel)){
                    tmp_flag_kel <- TRUE
                    flag_check <- TRUE
                  }
          ...
          ###if(comp_required[["AUCTAUi"]]) {
            auctau[[d]] <- auc_tau(conc = tmp_conc_di, time = tmp_time_di, method = method, exflag = auc_flag, told = told[[d]], tau = tau[[d]], curr_di = d, max_di = max_di, t_max = t_maxi[[d]], orig_conc = orig_conc, orig_time = orig_time, last_crit_factor = last_crit_factor, kel = kel_v, auclast = auclasti[[d]], lasttime = last_timei[[d]], orgtime = tolower(map_data$ORGTIME), nom_time = tmp_nom_time_di, ctoldest = nxt_ctold_est, flgacceptkel = tmp_flag_kel)
          ###}
          ###if(comp_required[["AUCTAUDNi"]]) {
            auctaudn[[d]] <- auc_dn(auc = auctau[[d]], dose = tmp_dose)
          ###}
          ###if(comp_required[["AUMCTAUi"]]){
            if(isTRUE(exists("auctau"))){
              if(isTRUE(is.na(auctau[[d]]) || is.null(auctau[[d]]))){
                aumctaui[[d]] <- NA
              } else {
                aumctaui[[d]] <- aumc_tau(conc = tmp_conc_di, time = aumc_time, method = method, exflag = auc_flag, told = told[[d]], tau = tau[[d]], curr_di = d, max_di = max_di, t_max = t_maxi[[d]], orig_conc = orig_conc, orig_time = orig_time, last_crit_factor = last_crit_factor, kel = kel_v, aumclast = aumclasti[[d]], lasttime = last_timei[[d]], orgtime = tolower(map_data$ORGTIME), nom_time = tmp_nom_time_di, ctoldest = nxt_ctold_est, flgacceptkel = tmp_flag_kel)
              }
            } else {
              aumctaui[[d]] <- aumc_tau(conc = tmp_conc_di, time = aumc_time, method = method, exflag = auc_flag, told = told[[d]], tau = tau[[d]], curr_di = d, max_di = max_di, t_max = t_maxi[[d]], orig_conc = orig_conc, orig_time = orig_time, last_crit_factor = last_crit_factor, kel = kel_v, aumclast = aumclasti[[d]], lasttime = last_timei[[d]], orgtime = tolower(map_data$ORGTIME), nom_time = tmp_nom_time_di, ctoldest = nxt_ctold_est, flgacceptkel = tmp_flag_kel)
            }
          ###}
          ###if(comp_required[["MRTLASTi"]]) {
            mrtlasti[[d]] <- mrt_last(conc = tmp_conc_di, time = aumc_time, method = method, model = "M2", aucflag = auc_flag, dof = dose_inf[[d]], auclast = auclasti[[d]], aumclast = aumclasti[[d]])
          ###}
          ###if(comp_required[["MRTIVIFOi"]]){
            mrtivifoi[[d]] <- mrt_ivif_o(conc = tmp_conc_di, time = aumc_time, method = method, model = "M2", parameter = "SS", kelflag = kel_flag, aucflag = auc_flag, tau = tau[[d]], orig_conc = orig_conc, orig_time = orig_time, aucinfo = aucinfoi[[d]], aumcinfo = aumcinfoi[[d]], auctau = auctau[[d]], aumctau = aumctaui[[d]])
          ###}
          ###if(comp_required[["MRTIVIFPi"]]){
            mrtivifpi[[d]] <- mrt_ivif_p(conc = tmp_conc_di, time = aumc_time, method = method, model = "M2", parameter = "SS", kelflag = kel_flag, aucflag = auc_flag, tau = tau[[d]], orig_conc = orig_conc, orig_time = orig_time, aucinfp = aucinfpi[[d]], aumcinfp = aumcinfpi[[d]], auctau = auctau[[d]], aumctau = aumctaui[[d]])
          ###}
          ###if(comp_required[["CAVi"]]) {
          ...
          ###}
          ###if(comp_required[["CTROUGHi"]]){
            c_troughi[[d]] <- ctrough(conc = tmp_conc_di, time = tmp_nom_time_di, tau = tau[[d]], told = told[[d]])
          ###}
          ###if(comp_required[["PTROUGHRi"]]){
          ...
          ###}
          if(isTRUE(dose_by_mass)){
            cl_tauw[[d]] <- cltauw(cltau = dose_c_i[[d]], normbs = auctau[[d]])
          } else {
            cl_tauw[[d]] <- cltauw(cltau = cl_tau[[d]], normbs = norm_bs)
          }
          ###if(comp_required[["PTFi"]]) {
          ...
          ###}
          if(isTRUE(dose_by_mass)){
            cl_ow <- clow(clo = dose_c_i[[d]], normbs = aucinfoi[[d]])
            vssow[[d]] <- mrtivifoi[[d]] * cl_ow
          } else {
            vssow[[d]] <- vssow(vsso = vsso[[d]], normbs = norm_bs)
          }
          ###if(comp_required[["VSSPWi"]]) {
          ...
          ###}
          if(isTRUE(dose_by_mass)){
            cl_pw <- clpw(clp = dose_c_i[[d]], normbs = aucinfpi[[d]])
            vsspw[[d]] <- mrtivifpi[[d]] * cl_pw
          } else {
            vsspw[[d]] <- vsspw(vssp = vssp[[d]], normbs = norm_bs)
          }
          ###if(comp_required[["VZTAUi"]]) {
          ...
          ###}
          if(isTRUE(dose_by_mass)){
            tmp_denom <- kel_v[["KEL"]] * auctau[[d]]
            vz_tauw[[d]] <- vzftauw(vzftau = dose_c_i[[d]], normbs = tmp_denom)
          } else {
            vz_tauw[[d]] <- vzftauw(vzftau = vz_tau[[d]], normbs = norm_bs)
          }  
          ###if((comp_required[["AUCT"]] || comp_required[["AUCTDN"]]) && auc_len > 1) {
            ...
            if(length(time) > 1){
              for(t in 2:(length(time))){
                if(time[t] %in% time_di[-1]) {
                  tmp <- auc_t1_t2(conc = auc_conc, time = auc_time, t1 = time_di[1], t2 = time[t], method = method, exflag = a_auc_flag, t_max = t_maxi[[d]])
            ...
            for(t in 1:(auc_par_len)){
              ...
              if(isTRUE(tmp_start_time <= orig_auc_t1 && orig_auc_t1 < tmp_end_time) || isTRUE(tmp_start_time < orig_auc_t2 && orig_auc_t2 <= tmp_end_time) || isTRUE(auct1_t2_check[[t]])){
                ...
                if(isTRUE(auct1_t2_check[[t]])){
                  if(isTRUE(d > 1 && orig_auc_t1 < tmp_start_time)){
                    auc_t1 <- tmp_start_time
                  }
                  if(isTRUE(d < di_col && orig_auc_t2 > tmp_end_time)){
                    auc_t2 <- tmp_end_time
                  }
                  
                  if((isTRUE(interpolation) || isTRUE(extrapolation))){
                    tmp <- auc_t1_t2(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_maxi[[d]], interpolate = interpolation, extrapolate = extrapolation, model = "M2", dosing_type = "SS", dosing_interval = d, told = tmp_told, prev_told = tmp_prev_told, prev_tau = tmp_prev_tau, last_crit_factor = last_crit_factor, kel = kel_v, orig_conc = orig_conc, orig_time = orig_time, orgtime = tolower(map_data$ORGTIME), includeNA = TRUE)
                    if(is.list(tmp)){  
                      tmp_auc <- tmp[[1]]
                      if(t == 1){
                        cest_tmp <- tmp[[2]]
                ...
                if(isTRUE(tmp_start_time < orig_auc_t2) && isTRUE(orig_auc_t2 <= tmp_end_time)){
        ...
        for(d in 1:di_col){
          ...
          if((d+1) <= di_col){
            if(!isTRUE(nxt_ctold_exists)){
              ###if(comp_required[["CTROUGHENDi"]]){
                c_troughendi[[d]] <- ctroughend(conc = tmp_conc_di, time = tmp_nom_time_di, tau = tau[[d]], told = told[[d]], ctold = next_ctold_est)
              ###}
              ###if(comp_required[["PTROUGHRENDi"]]){
        ...
        ####  aumcinf_o <- aumc_inf_o(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, aumclast = aumclast, c_last = c_last, t_last = t_last, kel = kel_v)
        ####}
        ####if(comp_required[["AUMCINFP"]]) {
        ####  aumcinf_p <- aumc_inf_p(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, aumclast = aumclast, t_last = t_last, kel = kel_v)
        ####}
          
        ###if(comp_required[["MRTLAST"]]) {
          mrtlast <- mrt_last(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], method = method, model = "M2", aucflag = auc_flag, dof = dose_inf[[d]], auclast = auclast, aumclast = aumclast)
        ###}
        ###if(comp_required[["AUCXPCTO"]]){
          aucxpcto <- auc_XpctO(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, auc_info = aucinf_o, auclast = auclast)
        ###}
        ###if(comp_required[["AUCXPCTP"]]){
          aucxpctp <- auc_XpctP(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, auc_infp = aucinf_p, auclast = auclast)
        ###}
          
        ####REMOVING AUMC PARAMETERS  
        ####if(comp_required[["AUMCXPTO"]]){
        ####  aumcxpto <- aumc_XpctO(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, aumcinfo = aumcinf_o, aumclast = aumclast)
        ####}
        ####if(comp_required[["AUMCXPTP"]]){
        ####  aumcxptp <- aumc_XpctP(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, aumcinfp = aumcinf_p, aumclast = aumclast)
        ####}
          
        ###if(comp_required[["AUCXBPCTO"]]){
          aucxbpcto <- auc_XbpctO(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, auc_info = aucinf_o)
        ###}
        ###if(comp_required[["AUCXBPCTP"]]){
          aucxbpctp <- auc_XbpctP(conc = auc_conc, time = auc_time, method = method, kelflag = a_kel_flag, aucflag = a_auc_flag, auc_infp = aucinf_p)
        ###}
  ...
  if(isTRUE(remove_extra_AUC)){
    auc_names <- c(paste0("AUC",auc_len), paste0("AUC",auc_len,"DN"), paste0("AUCINT",auc_len))
    computation_df <- computation_df[, !(names(computation_df) %in% auc_names)]
    regular_int_type <- regular_int_type[!(regular_int_type %in% auc_names)]
  }
      ...
      tryCatch({
        ...
        if(isTRUE(pkterm_parent %in% tmp_mr_df[,map_data$PKTERM])){
          ...
          for(d in 1:di_col){
            if(disp_required[["MRAUCTAUi"]]) {
              if(isTRUE(curr_pkterm != pkterm_parent)){
                mr_auctau <- mr_auc_tau(metabolite_auctau = tmp_df[,paste0("AUCTAU",d)], parent_auctau = tmp_parent_df[,paste0("AUCTAU",d)], parent_mw = tmp_parent_df$MW, metabolite_mw = tmp_df$MW)
                computation_df[computation_df[,map_data$SDEID] == sdeid, paste0("MRAUCTAU",d)] <- mr_auctau
              } 
            }
            if(disp_required[["MRCMAXi"]]) {
              if(isTRUE(curr_pkterm != pkterm_parent)){
