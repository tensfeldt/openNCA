...
#' @param sid The subject ID (either can be the id number or 'all' to get cmin for all the subjects)
...
#' cmin_i(mod)  
#' #Please specify for which subject you want to get the CMINi for!
#' 
#' cmin_i(mod, sid = "all")  
#' #"2.47", "ND", "1.19"
#' 
...
#' 
#' cmin_i(mod, sid = 31, interval = -1)
#' #Interval provided is less than zero!
#' 
#' cmin_i(mod, sid = 31, interval = 4)
#' #Interval provided is greater than maximum time!
#' 
#' cmin_i(mod, sid = 30, interval = 1)
#' #"2.498"
#' 
#' cmin_i(mod, sid = "all", interval = 4)
#' #Interval provided is greater than maximum time!
#' 
#' ##############
#' ## nca_data ##
...
#' }
#' @export
cmin_i <- function(data = NULL, map = NULL, sid = list(), interval = 0){
  if(is.null(data)){
  ...
  cmin_df <- data.frame(matrix(ncol = 3, nrow = 0))
  names(cmin_df) <- c("SDEID", "CMIN", "INTERVAL")
  ...
  if(!is.null(sid)){
    if(tolower(sid) == 'all' || (typeof(sid) == 'list' && length(sid) == 0)){
      for(i in 1:length(unique(data_data[,map_data$SDEID]))){
        ...
        if(c_min == 0 || is.infinite(c_min)){  
          cmin_df[i,] <- c(unique(data_data[,map_data$SDEID])[i], "ND", interval)  
        } else {
          cmin_df[i,] <- c(unique(data_data[,map_data$SDEID])[i], c_min, interval)
        }
      }
      return(cmin_df)
    } else {
