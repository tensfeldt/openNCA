#' Observed concentration at the end of infusion
#'
#' This function gets the observed concentration at the end of infusion. In the case of multiple dosing, CENDINFi
...
#' \strong{Value} \cr
#' \itemize{
#'  \item CENDINF: observed concentration at the end of infusion
#' }
#'
#' @examples
#' ##########
...
cendinf <- function(conc = NULL, time = NULL, dof = NULL, cmax = NULL){
  if(is.null(conc) && is.null(time) && is.null(dof)){
    ...
    stop("Error in cendinf: 'conc' and 'time' vectors are NULL")
  } else if(is.null(conc) && is.null(dof)){
    stop("Error in cendinf: 'conc' and 'dof' vectors are NULL")
  } else if(is.null(time) && is.null(dof)){
    stop("Error in cendinf: 'time' and 'dof' vectors are NULL")
  } else if(is.null(conc)) {
    stop("Error in cendinf: 'conc' vector is NULL")
  } else if(is.null(time)) {
    stop("Error in cendinf: 'time' vectors is NULL")
  } else if(is.null(dof)) {
    stop("Error in cendinf: 'dof' vectors is NULL")
  ...
  if(!(is.numeric(conc) && is.vector(conc)) ){
    stop("Error in cendinf: 'conc' is not a numeric vector")
  }
  if(!(is.numeric(time) && is.vector(time)) ){
    stop("Error in cendinf: 'time' is not a numeric vector")
  }
  if(length(time) != length(conc) ){
    stop("Error in cendinf: length of 'time' and 'conc' vectors are not equal")
  }
