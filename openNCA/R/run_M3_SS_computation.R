...
##
  ...
  if(isTRUE(optimize_kel) && (!"TMAXi" %in% parameter_list || !"TLASTi" %in% parameter_list || !"CMAXi" %in% parameter_list || !"CLASTi" %in% parameter_list || !"AUCLASTi" %in% parameter_list ||
    ...
    warning("Kel optimization cannot be performed because 'TMAXi', 'TLASTi', 'CMAXi', 'CLASTi', 'AUCLASTi' are not part of the calulcated parameters AND Flag 'FLGACCEPTKELCRIT' and Flag 'FLGXKEL' are not present in the dataset")
  }
...
##
              if((isTRUE(interpolation) || isTRUE(extrapolation))){
                tmp <- auc_t1_t2(conc = tmp_df[,map_data$CONC], time = na.omit(tmp_df[,map_data$TIME]), t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_max, interpolate = interpolation, extrapolate = extrapolation, model = "M3", dosing_type = "SS", told = tmp_told, kel = kel_v, orig_conc = orig_conc, orig_time = orig_time)
                if(is.list(tmp)){
                  tmp_auc <- tmp[[1]]
                  if(t == 1){
                    cest_tmp <- tmp[[2]]
