...
#' @param model_regex The regular expression that identifies the model and dosing type (default: "^M3(SS)*?$")
...
#' \strong{Dataframe} \cr
#' \itemize{
#'  \item M3SS_Parameters: Calculated default/specified parameters
#' }
#' OR \cr
#' \strong{List} \cr
#' \itemize{
...
run_M3_SS_computation <- function(data = NULL, map = NULL, method = 1, model_regex = "^M3(SS)*?$", parameter_list = list(), return_list = list(), raw_results_debug = FALSE, optimize_kel_debug = FALSE, ...){
  ...
  ###  disp_required[[i]] <- pr
  ###}
  all_parameters <- model_parameters("m3ss")
    ...
    tryCatch({
      ...
      if(isTRUE(nrow(tmp_df) > 0 & all(tmp_df[,map_data$TIME][!is.na(tmp_df[,map_data$TIME])] >= 0)) & isTRUE(time_check) & isTRUE(conc_check)){
        ...
        for(d in 1:di_col){
          ...
          ###}
          ###if(comp_required[["CENDINFi"]]){
            cend_inf[[d]] <- cendinf(conc = tmp_conc_di, time = tmp_time_di, dof = dose_inf[[d]], cmax = c_maxi[[d]])
          ###}
          ###if(comp_required[["CENDINFDNi"]]){
          ...
          ###if(comp_required[["TENDINFi"]]){
            if(toupper(map_data$ORGTIME) == "ACTUAL"){
              tend_inf[[d]] <- tendinf(conc = tmp_conc_di, time = tmp_time_di, dof = dose_inf[[d]], tmax = t_maxi[[d]], told = told[[d]])
            } else {
              tend_inf[[d]] <- tendinf(conc = tmp_conc_di, time = tmp_time_di, dof = dose_inf[[d]], tmax = t_maxi[[d]])
            }
          ###}
          ###if(comp_required[["CMINi"]]) {
          ...
          ###}
          ###if(comp_required[["MRTIVIFOi"]]){
            mrtivifoi[[d]] <- mrt_ivif_o(conc = tmp_conc_di, time = aumc_time, method = method, parameter = "SS", kelflag = kel_flag, aucflag = auc_flag, tau = tau[[d]], dof = dose_inf[[d]], orig_conc = tmp_df[,map_data$CONC], orig_time = tmp_df[,map_data$TIME], aucinfo = aucinfoi[[d]], aumcinfo = aumcinfoi[[d]], auctau = auctau[[d]], aumctau = aumctaui[[d]])
          ###}
          ###if(comp_required[["MRTIVIFPi"]]){
            mrtivifpi[[d]] <- mrt_ivif_p(conc = tmp_conc_di, time = aumc_time, method = method, parameter = "SS", kelflag = kel_flag, aucflag = auc_flag, tau = tau[[d]], dof = dose_inf[[d]], orig_conc = tmp_df[,map_data$CONC], orig_time = tmp_df[,map_data$TIME], aucinfp = aucinfpi[[d]], aumcinfp = aumcinfpi[[d]], auctau = auctau[[d]], aumctau = aumctaui[[d]])
          ###}
          ###if(comp_required[["CAVi"]]) {
              ...
              if(isTRUE(tmp_start_time <= orig_auc_t1 && orig_auc_t1 < tmp_end_time) || isTRUE(tmp_start_time < orig_auc_t2 && orig_auc_t2 <= tmp_end_time) || isTRUE(auct1_t2_check[[t]])){
                ...
                if(isTRUE(auct1_t2_check[[t]])){
                  ...
                  if((isTRUE(interpolation) || isTRUE(extrapolation))){
                    tmp <- auc_t1_t2(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_maxi[[d]], interpolate = interpolation, extrapolate = extrapolation, model = "M3", dosing_type = "SS", dosing_interval = d, told = tmp_told, prev_told = tmp_prev_told, prev_tau = tmp_prev_tau, last_crit_factor = last_crit_factor, kel = kel_v, orig_conc = orig_conc, orig_time = orig_time, orgtime = tolower(map_data$ORGTIME), includeNA = TRUE)
                    if(is.list(tmp)){  
                      tmp_auc <- tmp[[1]]
                      if(t == 1){
                        cest_tmp <- tmp[[2]]
        ...
        if(disp_required[["DOFi"]]){
          computation_df[i, paste0("DOF",1:di_col)] <- unlist(dose_inf)
        }
        if(disp_required[["CENDINFi"]]){
