...
run_M3_SS_computation <- function(data = NULL, map = NULL, method = 1, model_regex = "^M3(SS)*?$", parameter_list = list(), return_list = list()){
...
###    if("AUCDN" %in% display_list && "AUCALL" %in% display_list) {
### 2019-10-10/TGT/ replace AUCDN with AUCALLDN
###    if(disp_required[["AUCDN"]]) {
...
###    if("AUCLASTi" %in% display_list) {
    if(disp_required[["AUCLASTi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCLAST",1:di_col)))
    }
###    if("AUCLASTDNi" %in% display_list) {
    if(disp_required[["AUCLASTDNi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCLASTDN",1:di_col)))
    }
...
### 2019-08-29/TGT/ include auc_pair_check to validate whether there are appropriate AUCpairs
###    if("AUCT1_T2" %in% display_list) {
...
###    if("MRTIVIFOi" %in% display_list){
    if(disp_required[["MRTIVIFOi"]]){
      display_parameters <- c(display_parameters, rep(paste0("MRTIVIFO",1:di_col)))
    }
###    if("MRTIVIFPi" %in% display_list){
    if(disp_required[["MRTIVIFPi"]]){
      display_parameters <- c(display_parameters, rep(paste0("MRTIVIFP",1:di_col)))
    }
...
###    if("CLTAUi" %in% display_list) {
    if(disp_required[["CLTAUi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CLTAU",1:di_col)))
    }
###    if("CLTAUWi" %in% display_list) {
    if(disp_required[["CLTAUWi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CLTAUW",1:di_col)))
    }
...
###    if("VZO" %in% display_list) {
    if(disp_required[["VZO"]]) {
      display_parameters <- c(display_parameters, rep(paste0("VZO",1:di_col)))
    }
###    if("VZP" %in% display_list) {
    if(disp_required[["VZP"]]) {
      display_parameters <- c(display_parameters, rep(paste0("VZP",1:di_col)))
    }
