...
#'
#' This function will compute all the relevant parameters for a M1 model Steady State (SS).\cr
#'
#' @details
...
#' @param model_regex The regular expression that identifies the model and dosing type (default: "^M1(SS)*?$")
...
#' \strong{Dataframe} \cr
#' \itemize{
#'  \item M1SS_Parameters: Calculated default/specified parameters
#' }
#' OR \cr
#' \strong{List} \cr
#' \itemize{
...
#' }
#' @export
run_M1_SS_computation <- function(data = NULL, map = NULL, method = 1, model_regex = "^M1(SS)*?$",  parameter_list = list(), return_list = list(), raw_results_debug = FALSE, optimize_kel_debug = FALSE, ...){
  ...
  ###  disp_required[[i]] <- pr
  ###}
  all_parameters <- model_parameters("m1ss")
    ...
    tryCatch({
      ...
      if(isTRUE(nrow(tmp_df) > 0 & all(tmp_df[,map_data$TIME][!is.na(tmp_df[,map_data$TIME])] >= 0)) & isTRUE(time_check) & isTRUE(conc_check)){
        ...
        for(d in 1:di_col){
          ...
          ###if(comp_required[["AUMCLAST"]]) {
            #FEEDBACK: Removed CONC/CONCTIME Based of 3.10 scope item
            #tmp_aumclast[[d]] <- aumc_last(conc = tmp_di_df[,map_data$CONC], time = aumc_time, method = method, exflag = auc_flag, t_max = t_maxi[[d]])
          ...
          if(all(c("KELNOPT", "KELRSQ") %in% flag_df$VAR) && ("AUCXPCTO" %in% flag_df$VAR || "AUCXPCTP" %in% flag_df$VAR)){
            ...
            if(length(ulist) >= 1){
              for(k in 1:length(ulist)){
                ...
                auclast_opt <- auc_last(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], method = method, exflag = auc_flag, t_last = t_last, t_max = t_max)
                span_ratio <- ifelse("SPANRATIOCRIT" %in% names(map_data), suppressWarnings(as.numeric(map_data$SPANRATIOCRIT)), NA)
                
                if("AUCXPCTO" %in% flag_df$VAR){
                  aucinfo_opt <- auc_inf_o(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], method = method, auclast = auclast_opt, c_last = c_last, spanratio = span_ratio, kel = all_kel)
                  ...
                  aucinfp_opt <- auc_inf_p(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], method = method, auclast = auclast_opt, t_last = t_last, spanratio = span_ratio, kel = all_kel)
        ...
        for(d in 1:di_col){
          ...
          ###}
          ###if(comp_required[["AUMCLASTi"]]){
            aumclasti[[d]] <- aumc_last(conc = tmp_conc_di, time = aumc_time, method = method, exflag = auc_flag, t_last = t_lasti[[d]], t_max = t_maxi[[d]])
          ###}
          ###if(comp_required[["AUCINFOi"]]){
            aucinfoi[[d]] <- auc_inf_o(conc = tmp_conc_di, time = tmp_time_di, method = method, kelflag = kel_flag, aucflag = auc_flag, c_last = c_lasti[[d]], auclast = auclasti[[d]], kel = kel_v)
          ###}
          ###if(comp_required[["AUCINFOCi"]]){
          ...
          ###}
          ###if(comp_required[["AUCINFPi"]]){
            aucinfpi[[d]] <- auc_inf_p(conc = tmp_conc_di, time = tmp_time_di, method = method, kelflag = kel_flag, aucflag = auc_flag, t_last = t_lasti[[d]], auclast = auclasti[[d]], kel = kel_v)
          ###}
          ###if(comp_required[["AUCINFPCi"]]){
          ...
          ###}
          ###if(comp_required[["AUMCINFOi"]]){
            aumcinfoi[[d]] <- aumc_inf_o(conc = tmp_conc_di, time = aumc_time, method = method, kelflag = kel_flag, aucflag = auc_flag, aumclast = aumclasti[[d]], c_last = c_lasti[[d]], t_last = t_lasti[[d]], kel = kel_v)
          ###}
          ###if(comp_required[["AUMCINFPi"]]){
          ...
          ###}
          ###if(comp_required[["MRTLASTi"]]){
            mrtlasti[[d]] <- mrt_last(conc = tmp_conc_di, time = aumc_time, method = method, model = "M1", aucflag = auc_flag, dof = dose_inf[[d]], auclast = auclasti[[d]], aumclast = aumclasti[[d]])
          ###}
          ###if(comp_required[["MRTEVIFOi"]]){
            mrtevifoi[[d]] <- mrt_evif_o(conc = tmp_conc_di, time = aumc_time, method = method, parameter = "SS", kelflag = kel_flag, aucflag = auc_flag, tau = tau[[d]], orig_conc = orig_conc, orig_time = orig_time, aucinfo = aucinfoi[[d]], aumcinfo = aumcinfoi[[d]], auctau = auctau[[d]], aumctau = aumctaui[[d]])
          ###}
          ###if(comp_required[["MRTEVIFPi"]]){
            mrtevifpi[[d]] <- mrt_evif_p(conc = tmp_conc_di, time = aumc_time, method = method, parameter = "SS", kelflag = kel_flag, aucflag = auc_flag, tau = tau[[d]], orig_conc = orig_conc, orig_time = orig_time, aucinfp = aucinfpi[[d]], aumcinfp = aumcinfpi[[d]], auctau = auctau[[d]], aumctau = aumctaui[[d]])
          ###}
          ###if(comp_required[["CAVi"]]){
          ...
          ###}
          if(isTRUE(dose_by_mass)){
            clf_tauw[[d]] <- clftauw(clftau = dose_ci[[d]], normbs = auctau[[d]])
          } else {
            clf_tauw[[d]] <- clftauw(clftau = clf_tau[[d]], normbs = norm_bs)
          }
          ###if(comp_required[["PTFi"]]){
          ...
          if(isTRUE(dose_by_mass)){
            ...
            vzf_tauw[[d]] <- vzftauw(vzftau = dose_ci[[d]], normbs = tmp_denom)
          } else {
            vzf_tauw[[d]] <- vzftauw(vzftau = vzf_tau[[d]], normbs = norm_bs)
          }  
              ...
              if(isTRUE(tmp_start_time <= orig_auc_t1 && orig_auc_t1 < tmp_end_time) || isTRUE(tmp_start_time < orig_auc_t2 && orig_auc_t2 <= tmp_end_time) || isTRUE(auct1_t2_check[[t]])){
                ...
                if(isTRUE(auct1_t2_check[[t]])){
                  ...
                  if((isTRUE(interpolation) || isTRUE(extrapolation))){
                    tmp <- auc_t1_t2(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_maxi[[d]], interpolate = interpolation, extrapolate = extrapolation, model = "M1", dosing_type = "SS", dosing_interval = d, told = tmp_told, prev_told = tmp_prev_told, prev_tau = tmp_prev_tau, last_crit_factor = last_crit_factor, kel = kel_v, orig_conc = orig_conc, orig_time = orig_time, orgtime = tolower(map_data$ORGTIME), includeNA = TRUE)
                    if(is.list(tmp)){  
                      tmp_auc <- tmp[[1]]
                      if(t == 1){
                        cest_tmp <- tmp[[2]]
                    ...
                    tmp_auc <- auc_t1_t2(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_maxi[[d]], model = "M1", dosing_type = "SS", includeNA = TRUE)
                  }
                } else {
                  tmp_auc <- 0
                }
        ...
        ###if(comp_required[["MRTLAST"]]){
          mrtlast <- mrt_last(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], method = method, model = "M1", aucflag = auc_flag, dof = dose_inf[[d]], auclast = auclast, aumclast = aumclast)
        ###}
