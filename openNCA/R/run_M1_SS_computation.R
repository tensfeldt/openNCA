...
#' }
#' @export
run_M1_SS_computation <- function(data = NULL, map = NULL, method = 1, model = "M1", parameter = "SS", return = list()){
  if(is.null(data)){
  ...
  if(tolower(return) == 'all' ||  (typeof(return) == 'list' && length(return) == 0)){
    ...
    for(i in 1:length(unique(data_data[,map_data$SDEID]))){
      ...
      for(d in 1:di_col){
        ...
        dose[[d]] <- tmp_dose
        cl_o[[d]] <- clo(aucinfo = aucinf_o, dose = tmp_dose)
        clf_o[[d]] <- clfo(aucinfo = aucinf_o, dose = tmp_dose)
        ...
        clf_tauw[[d]] <- clftauw(clftau = clf_tau[[d]], normbs = tmp_di_df[,map_data$NORMBS][1])
        ...
        vzf_tauw[[d]] <- vzftauw(vzftau = vzf_tau[[d]], normbs = tmp_di_df[,map_data$NORMBS][1])
