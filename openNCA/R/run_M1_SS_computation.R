              ...
              if(isTRUE(tmp_start_time <= orig_auc_t1 && orig_auc_t1 < tmp_end_time) || isTRUE(tmp_start_time < orig_auc_t2 && orig_auc_t2 <= tmp_end_time) || isTRUE(auct1_t2_check[[t]])){
                ...
                if(isTRUE(auct1_t2_check[[t]])){
                  ...
                  if((isTRUE(interpolation) || isTRUE(extrapolation))){
                    tmp <- auc_t1_t2(conc = tmp_df[,map_data$CONC], time = tmp_df[,map_data$TIME], t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_maxi[[d]], interpolate = interpolation, extrapolate = extrapolation, model = "M1", dosing_type = "SS", told = tmp_told, kel = kel_v, orig_conc = orig_conc, orig_time = orig_time, includeNA = TRUE)
                    if(is.list(tmp)){  
                      tmp_auc <- tmp[[1]]
                      if(t == 1){
                        cest_tmp <- tmp[[2]]
        ...
        for(d in 1:di_col){
          ...
          if((d+1) <= di_col){
            ...
            if(!isTRUE(next_ctold_exists)){
              ...
              next_est_tmp <- estimate_told_concentration(conc = next_tmp_conc_di, time = next_tmp_time_di, interpolate = TRUE, extrapolate = TRUE, auc_method = "LIN", model = "M1", dosing_type = "SS", told = next_tmp_told, orig_conc = orig_conc, orig_time = orig_time)
