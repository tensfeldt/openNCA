...
###        if(parameter_required("^AUCT(DN)*?$", parameter_list) || length(dependent_parameters("^AUCT(DN)*?$"))>0){
        if(comp_required[["AUCTDN"]] || comp_required[["AUCT"]]){
          auc_int <- list()
        }
###        if("AUCT1_T2" %in% parameter_list && 'TMAXi' %in% parameter_list && auc_pair_check) {
###        if((parameter_required("^AUCT1_T2$", parameter_list) || length(dependent_parameters("^AUCT1_T2$"))>0) && auc_pair_check){
...
###          if(parameter_required("^AUCT1_T2$", parameter_list) && auc_pair_check) {
          if(comp_required[["AUCT1_T2"]] && auc_pair_check) {
            ...
            for(t in 1:(auc_par_len)){
              ...
              tmp <- auc_t1_t2(conc = tmp_di_df[,map_data$CONC], time = na.omit(tmp_di_df[,map_data$TIME]), t1 = auc_t1, t2 = auc_t2, method = method, exflag = auc_flag, t_max = t_maxi[[d]])

              if(d == 1){
                auct1_t2[[t]] <- tmp
              } else {
                auct1_t2[[t]] <- sum(c(unlist(auct1_t2[[t]]), tmp), na.rm = TRUE)
              }
            }
          }
...
### 2019-09-05/TGT/ 
###        if("LASTTIMEACCEPTCRIT" %in% names(map_data) && ("LASTTIME" %in% parameter_list)) {
        if(parameter_required("LASTTIMEACCEPTCRIT", names(map_data)) && parameter_required("LASTTIME", parameter_list)) {
...
### 2019-08-18/TGT/ PARAMETERDISPLAYLIST/display_list should be managed in run_computation.R not in the individual run_Mx_Sx_computation.R routines or call a function
###                  this manage the list more appropriately
if(FALSE) {    
  if(is.list(display_list) && !is.null(display_list) && length(display_list) > 0){
    display_parameters <- c("SDEID")
###    if("CMAX" %in% display_list) {
    if(disp_required[["CMAX"]]) {
      display_parameters <- c(display_parameters, "CMAX")
    }
###    if("CMAXi" %in% display_list) {
    if(disp_required[["CMAXi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CMAX",1:di_col)))
    }
### 2019-08-30/TGT/ Remove "CMAXC" since only valid for M2 Bolus administration applications
###    if("CMAXC" %in% display_list) {
###      display_parameters <- c(display_parameters, "CMAXC")
###    }
### 2019-08-30/TGT/ Remove "CMAXCi" since only valid for M2 Bolus administration applications
###    if("CMAXCi" %in% display_list) {
###      display_parameters <- c(display_parameters, rep(paste0("CMAXC",1:di_col)))
###    }
###    if("CMAXDNi" %in% display_list) {
    if(disp_required[["CMAXDNi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CMAXDN",1:di_col)))
    }
    if("FLGACCEPTPREDOSECRIT" %in% names(map_data)){
      display_parameters <- c(display_parameters, "FLGACCEPTPREDOSE")
    }
###    if("CMIN" %in% display_list) {
    if(disp_required[["CMIN"]]) {
      display_parameters <- c(display_parameters, "CMIN")
    }
###    if("CMINi" %in% display_list) {
    if(disp_required[["CMINi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CMIN",1:di_col)))
    }
###    if("CLAST" %in% display_list) {
    if(disp_required[["CLAST"]]) {
      display_parameters <- c(display_parameters, "CLAST")
    }
###    if("CLASTi" %in% display_list) {
    if(disp_required[["CLASTi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CLAST",1:di_col)))
    }
###    if("TMAX" %in% display_list) {
    if(disp_required[["TMAX"]]) {
      display_parameters <- c(display_parameters, "TMAX")
    }
###    if("TMAXi" %in% display_list) {
    if(disp_required[["TMAXi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("TMAX",1:di_col)))
    }
    if("FLGEMESIS" %in% names(map_data)){
      display_parameters <- c(display_parameters, "FLGACCEPTTMAX")
    }
###    if("TMIN" %in% display_list) {
    if(disp_required[["TMIN"]]) {
      display_parameters <- c(display_parameters, "TMIN")
    }
###    if("TMINi" %in% display_list) {
    if(disp_required[["TMINi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("TMIN",1:di_col)))
    }
###    if("TLAST" %in% display_list) {
    if(disp_required[["TLAST"]]) {
      display_parameters <- c(display_parameters, "TLAST")
    }
###    if("TLASTi" %in% display_list) {
    if(disp_required[["TLASTi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("TLAST",1:di_col)))
    }
...
### 2019-10-10/TGT/ replace AUCDN with AUCALLDN
###    if(disp_required[["AUCDN"]]) {
###      display_parameters <- c(display_parameters, rep(paste0("AUCDN",1:di_col)))
    if(disp_required[["AUCALLDN"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCALLDN",1:di_col)))
    }
###    if("AUCLASTi" %in% display_list) {
    if(disp_required[["AUCLASTi"]]) {
      display_parameters <- c(display_parameters, "AUCLAST")
    }
###    if("AUCLASTDNi" %in% display_list) {
    if(disp_required[["AUCLASTDNi"]]) {
      display_parameters <- c(display_parameters, "AUCLASTDN")
    }
###    if("AUCT" %in% display_list) {
    if(disp_required[["AUCT"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUC",1:auc_len)))
    }
###    if("AUCTDN" %in% display_list) {
### 2019-10-10/TGT/ replace AUCDN with AUCALLDN
    if(disp_required[["AUCTDN"]]) {
###      display_parameters <- c(display_parameters, rep(paste0("AUCDN",1:auc_len)))
      display_parameters <- c(display_parameters, rep(paste0("AUCALLDN",1:auc_len)))
    }
###    if("AUCT" %in% display_list || "AUCTDN" %in% display_list) {
    if(disp_required[["AUCT"]] || disp_required[["AUCTDN"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCINT",1:auc_len)))
    }
### 2019-08-22/TGT/ added auc_pair_check condition to prevent failing if AUCT1_T2 was added into parameterlist but AUCNPAIR==0
###    if("AUCT1_T2" %in% display_list && auc_pair_check) {
    if(disp_required[["AUCT1_T2"]] && auc_pair_check) {
      tmp <- rep(paste0("AUC", map_data[,rep(paste0("AUC.", 1:auc_par_len, ".T1"))], "_", map_data[,rep(paste0("AUC.", 1:auc_par_len, ".T2"))]))
...
###    if("AUCINFO" %in% display_list) {
    if(disp_required[["AUCINFO"]]) {
      display_parameters <- c(display_parameters, "AUCINFO")
    }
###    if("AUCINFOi" %in% display_list) {
    if(disp_required[["AUCINFOi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCINFO",1:di_col)))
    }
###    if("AUCINFOC" %in% display_list) {
    if(disp_required[["AUCINFOC"]]) {
      display_parameters <- c(display_parameters, "AUCINFOC")
    }
###    if("AUCINFODN" %in% display_list) {
    if(disp_required[["AUCINFODN"]]) {
      display_parameters <- c(display_parameters, "AUCINFODN")
    }
###	if("CEST" %in% display_list) {
    if(disp_required[["CEST"]]) {
      display_parameters <- c(display_parameters, "CEST")
    }
###    if("AUCINFP" %in% display_list) {
    if(disp_required[["AUCINFP"]]) {
      display_parameters <- c(display_parameters, "AUCINFP")
    }
###    if("AUCINFPi" %in% display_list) {
    if(disp_required[["AUCINFPi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCINFP",1:di_col)))
    }
###    if("AUCINFPC" %in% display_list) {
    if(disp_required[["AUCINFPC"]]) {
      display_parameters <- c(display_parameters, "AUCINFPC")
    }
###    if("AUCINFPDN" %in% display_list) {
    if(disp_required[["AUCINFPDN"]]) {
      display_parameters <- c(display_parameters, "AUCINFPDN")
    }
###    if("AUCTAUi" %in% display_list) {
    if(disp_required[["AUCTAUi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCTAU",1:di_col)))
    }
###    if("AUCTAUDNi" %in% display_list) {
    if(disp_required[["AUCTAUDNi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUCTAUDN",1:di_col)))
    }
###    if("AUMCTAUi" %in% display_list) {
    if(disp_required[["AUMCTAUi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("AUMCTAU",1:di_col)))
    }
###    if("MRTEVIFOi" %in% display_list){
    if(disp_required[["MRTEVIFOi"]]){
      display_parameters <- c(display_parameters, rep(paste0("MRTEVIFO",1:di_col)))
    }
###    if("MRTEVIFPi" %in% display_list){
    if(disp_required[["MRTEVIFPi"]]){
      display_parameters <- c(display_parameters, rep(paste0("MRTEVIFP",1:di_col)))
    }
###    if("AUCXPCTO" %in% display_list){
    if(disp_required[["AUCXPCTO"]]){
      display_parameters <- c(display_parameters, "AUCXPCTO")
    }
###    if("AUCXPCTOi" %in% parameter_list){
    if(disp_required[["AUCXPCTOi"]]){
      display_parameters <- c(display_parameters, rep(paste0("AUCXPCTO",1:di_col)))
    }
###    if("AUCXPCTP" %in% display_list){
    if(disp_required[["AUCXPCTP"]]){
      display_parameters <- c(display_parameters, "AUCXPCTP")
    }
###    if("AUCXPCTPi" %in% parameter_list){
    if(disp_required[["AUCXPCTPi"]]){
      display_parameters <- c(display_parameters, rep(paste0("AUCXPCTP",1:di_col)))
    }
###    if("AUMCXPTO" %in% display_list){
    if(disp_required[["AUMCXPTO"]]){
      display_parameters <- c(display_parameters, "AUMCXPTO")
    }
###    if("AUMCXPTOi" %in% parameter_list){
    if(disp_required[["AUMCXPTOi"]]){
      display_parameters <- c(display_parameters, rep(paste0("AUMCXPTO",1:di_col)))
    }
###    if("AUMCXPTP" %in% display_list){
    if(disp_required[["AUMCXPTP"]]){
      display_parameters <- c(display_parameters, "AUMCXPTP")
    }
###    if("AUMCXPTPi" %in% parameter_list){
    if(disp_required[["AUMCXPTPi"]]){
      display_parameters <- c(display_parameters, rep(paste0("AUMCXPTP",1:di_col)))
    }
###    if("CAVi" %in% display_list) {
    if(disp_required[["CAVi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CAV",1:di_col)))
    }
###    if("CLFTAUi" %in% display_list) {
    if(disp_required[["CLFTAUi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CLFTAU",1:di_col)))
    }
###    if("CLFTAUWi" %in% display_list) {
    if(disp_required[["CLFTAUWi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("CLFTAUW",1:di_col)))
    }
###    if("PTFi" %in% display_list) {
    if(disp_required[["PTFi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("PTF",1:di_col)))
    }
###    if("PTRi" %in% display_list) {
    if(disp_required[["PTRi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("PTR",1:di_col)))
    }
###    if("VZFTAUi" %in% display_list) {
    if(disp_required[["VZFTAUi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("VZFTAU",1:di_col)))
    }
###    if("VZFTAUWi" %in% display_list) {
    if(disp_required[["VZFTAUWi"]]) {
      display_parameters <- c(display_parameters, rep(paste0("VZFTAUW",1:di_col)))
    }
...
###    cat('ku-names: ', ku, '\n')
### Note need to pop off TAU if it gets added
    if(length(grep("TAUU", ku, ignore.case=TRUE, perl=TRUE))>0) { ku <- ku[-match("TAUU", ku)] }

### 2019-08-18/TGT/  move name selection here
###    print(x)
###    print(names(x))
###    k <- c(names(computation_df), names(x))
...
###    cat('names being removed: ', j, '\n')
### temporarily suspend removing names
    x <- x[,-match(j, names(x))]
    # names of x are the entries to add back to computation_df
### 2019-08-18/TGT/ Following assumins only returning display_parameters but these should be managed in run_computation.R not here
