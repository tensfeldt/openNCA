...
#' }
#' @export
run_M2_SD_computation <- function(data = NULL, map = NULL, flag = NULL, method = 1, model = "M2", parameter = "SD", return = list()){
  if(is.null(data)){
  ...
  computation_list <- list("cmax", "clast", "cmax_dn", "tmax", "tlast", "kel", 
                           "kelr", "auc_all", "auc_dn", "auc_last", "auc_last_dn", "aumc_last", "auc_t1_t2", 
                           ...
                           "aumc_xpct_o", "aumc_xpct_p", "clow", "clp", "clpw", "vzo", "vzow", "vzp",
                           "vzpw")
  ...
  if(tolower(return) == 'all' ||  (typeof(return) == 'list' && length(return) == 0)){
    ...
    names(computation_df) <- c("SDEID", "CMAX", "CLAST", "CMAXDN", "TMAX", "TLAST", "KEL", "KELTMLO", "KELTHMI", 
                               "KELNOPT", "KELR", "KELRSQ", "KELRSQA", "THALF", "AUCALL", "AUCDN", "AUCLAST", "AUCLASTDN", "AUMCLAST", 
                               rep(paste0("AUC",1:auc_col)), rep(paste0("AUCINT",1:auc_col)), "AUCINFO", "AUCINFP", "AUCINFOC", "AUCINFPC", 
                               "AUCINFODN", "AUCINFPDN","AUMCINFO", "AUMCINFP", "MRTLAST", "MRTO", "MRTP", "AUCXPCTO", "AUCXPCTP", 
                               "AUMCXPCTO", "AUMCXPCTP", "CLOW", "CLP", "CLPW", "VZO", "VZOW", "VZP", "VZPW")
    ...
    for(i in 1:length(unique(data_data[,map_data$SDEID]))){
      ...
      computation_df[i,] <- c(unique(data_data[,map_data$SDEID])[i], c_max, c_last, cmaxdn, t_max, t_last,
                              kel_v[["KEL"]], kel_v[["KELTMLO"]], kel_v[["KELTMHI"]], kel_v[["KELNOPT"]], kelr_v[["KELR"]], 
                              kelr_v[["KELRSQ"]], kelr_v[["KELRSQA"]], kel_v[["THALF"]], aucall, aucdn, auclast, auclastdn, 
                              aumclast, auct, auc_int, aucinf_o, aucinf_p, aucinf_oc, aucinf_pc, aucinfo_dn, aucinfp_dn, 
                              aumcinf_o, aumcinf_p, mrtlast, mrto, mrtp, aucxpcto, aucxpctp, aumcxpcto, aumcxpctp, cl_ow, cl_p, 
                              cl_pw, vz_o, vz_ow, vz_p, vz_pw)
    }
  } else {
    if(typeof(return) == "list" && !any(!return %in% computation_list)) {
      ...
      for(i in 1:length(unique(data_data[,map_data$SDEID]))){
        ...
        if("clow" %in% return){
          cl_o <- clo(aucinfo = aucinf_o, dose = tmp_df[,map_data$DOSE][i])
          cl_ow <- clow(clo = cl_o, normbs = tmp_df[,map_data$NORMBS][i])
          row_data <- append(row_data, cl_ow)
        }
        if("clp" %in% return){
          cl_p <- clp(aucinfp = aucinf_p, dose = tmp_df[,map_data$DOSE][i])
          row_data <- append(row_data, cl_p)
        }
        if("clp" %in% return){
          cl_pw <- clpw(clp = cl_p, normbs = tmp_df[,map_data$NORMBS][i])
          row_data <- append(row_data, cl_pw)
        }
        if("vzo" %in% return){
          vz_o <- vzo(kel = kel_v, aucinfo = aucinf_o, dose = tmp_df[,map_data$DOSE][i])
          row_data <- append(row_data, vz_o)
        }
        if("vzow" %in% return){
          vz_ow <- vzow(vzo = vz_o, normbs = tmp_df[,map_data$NORMBS][i])
          row_data <- append(row_data, vz_ow)
        }
        if("vzp" %in% return){
          vz_p <- vzp(kel = kel_v, aucinfp = aucinf_p, dose = tmp_df[,map_data$DOSE][i])
          row_data <- append(row_data, vz_p)
        }
        if("vzpw" %in% return){
          vz_pw <- vzpw(vzp = vz_p, normbs = tmp_df[,map_data$NORMBS][i])
          row_data <- append(row_data, vz_pw)
        }
