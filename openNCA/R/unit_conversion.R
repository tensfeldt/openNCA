...
#' @param unit_class The value that determines for which unit class unit conversion will be applied (default: 'ALL')
...
unit_conversion <- function(data = NULL, map = NULL, result = NULL, unit_class = "ALL", verbose=TRUE){
  ...
  if(unit_class == "TIMEU" || unit_class == "ALL"){
    ...
    if(length(TIMEUPARAM)>0 && parameter_required("^TIMEU$", names(map_data))){
      if(parameter_required(map_data$TIMEU, names(data_data))){
        inputUnit1 <- toupper(as.character(unique(data_data[, map_data$TIMEU])[1]))
        ...
        if(testunit) {
          outputUnit1 <- toupper(as.character(map_data[[outputUnitLabel]]))
    ...
    if(length(AMOUNTUPARAM)>0 && parameter_required("^AMOUNTU$",names(map_data)) && parameter_required("^CONCU$",names(map_data))){
      if(parameter_required(map_data$AMOUNTU, names(data_data))){
        ...
        if(length(grep("/", as.character(unique(data_data[, map_data$CONCU])))) > 0){
          inputUnit2 <- toupper(as.character(conc_unit_tmp[1]))
        } else {
          inputUnit2 <- NA
        }
        ...
        if(testunit) {
            outputUnit2 <- toupper(as.character(map_data[[outputUnitLabel]]))
      ...
      if(interval_len > 1){
            ...
            if(parameter_required(map_data[, doseuvar[i]], names(data_data))) {
              ...
              if(length(unique(data_data[, as.character(map_data[,doseuvar])])) == 1){
                if(length(grep("/", as.character(unique(data_data[, as.character(map_data[,doseuvar])])[1])) > 0)){
                  inputUnit3 <- toupper(as.character(unlist(strsplit(as.character(unique(data_data[, as.character(map_data[,doseuvar[i]])])[1]), "/"))))
                  dose_mass_input <- TRUE
                  dose_mass_output <- FALSE
                } else {
                  inputUnit3 <- toupper(as.character(unique(data_data[, map_data[, doseuvar[i]]])[1]))
              ...
              if(testunit) {
                outputUnit3 <- toupper(as.character(map_data[[outputUnitLabel]]))
              ...
              if(testunit){
                if(isTRUE(dose_mass_input) && isTRUE(dose_mass_output)){
                  ...
                  for(i in 1:2) {
                    if(!all(is.na(outputUnit3))){
                      ...
                      if(inputMatch3[i] != missing_idx && outputMatch3[i] != missing_idx) {
                        ...
                        if(i == 2 && inputUnit3[i] %in% dose_denominator && outputUnit3[i] %in% dose_denominator) {
                          doseUScaler[i] <- 1
                  ...
                  if(testunit) { testunit <- testunit && valid_input_unit && valid_output_unit && valid_conversion }
                  if(testunit){
        ...
        if(parameter_required(doseuvar, names(map_data))){
          if(parameter_required(map_data[,doseuvar], names(data_data))){
            if(length(unique(data_data[, as.character(map_data[,doseuvar])])) == 1){
              if(length(grep("/", as.character(unique(data_data[, as.character(map_data[,doseuvar])])[1])) > 0)){
                inputUnit3 <- toupper(as.character(unlist(strsplit(as.character(unique(data_data[, as.character(map_data[,doseuvar])])[1]), "/"))))
                dose_mass_input <- TRUE
                dose_mass_output <- FALSE
              } else {
                inputUnit3 <- toupper(as.character(unique(data_data[, as.character(map_data[,doseuvar])])[1]))
                dose_mass_input <- FALSE
                dose_mass_output <- FALSE
              }
            } else {
                ...
                for(i in 1:2) {
                  if(!all(is.na(outputUnit3))){
                    ...
                    if(inputMatch3[i] != missing_idx && outputMatch3[i] != missing_idx) {
                      ...
                      } else if(i == 2 && (inputUnit3[i] %in% dose_denominator && !(outputUnit3[i] %in% dose_denominator)) || (!(inputUnit3[i] %in% dose_denominator) && outputUnit3[i] %in% dose_denominator)) {
                        doseUScaler[i] <- 1
                        valid_conversion <- FALSE
                        warning("'", inputUnit3[i], "' to '", outputUnit3[i], "' DOSEU conversion is not accounted for unit conversion")
                      } else {
  ...
  if(unit_class == "VOLUMEU" || unit_class == "ALL"){
    ...
    if(length(VOLUMEUPARAM)>0 && parameter_required("^CONCU$", names(map_data))){
      if(map_data$CONCU %in% names(data_data)){
        ...
        inputUnit4 <- ifelse(length(as.character(volume_unit_tmp)) == 2, toupper(as.character(volume_unit_tmp[2])), NA)
        ...
        if(testunit) {
            outputUnit4 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "CONCU" || unit_class == "ALL"){
    ...
    if(length(CONCUPARAM)>0 & parameter_required("^CONCU$", names(map_data))){
      if(parameter_required(map_data$CONCU, names(data_data))){
        ...
        if(length(grep("/", as.character(unique(data_data[, map_data$CONCU])))) > 0){
          inputUnit5 <- toupper(as.character(conc_unit_tmp))
        } else {
          inputUnit5 <- NA
        }
        ...
        if(testunit) {
            outputUnit5 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "KELU" || unit_class == "ALL"){
    KELUPARAM <- parameters_by_class("KELU", names(result_data))
      ...
      if(length(KELUPARAM)>0 && (parameter_required(map_data$TIME, names(data_data)) &&
        ...
        inputUnit6 <- NA
        outputUnit6 <- NA
        if(parameter_required(map_data$TIMEU, names(data_data))) { inputUnit6 <- toupper(as.character(unique(data_data[,map_data$TIMEU]))) }  
        ...
        numerator <- "1"
        if(testunit) {
            baseUnit6 <- outputUnit6 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "CLU" || unit_class == "ALL"){
    ...
    if(length(CLUPARAM)>0 && parameter_required("^TIMEU$", names(map_data)) && parameter_required("^CONCU$", names(map_data))){
      if(parameter_required(map_data$TIMEU, names(data_data)) && parameter_required(map_data$CONCU, names(data_data))){
        ...
        inputUnit7 <- toupper(c(as.character(clu_unit_tmp[2]), as.character(unique(data_data[, map_data$TIMEU])[[1]])))
        ...
        if(testunit) {
            outputUnit7 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "AUCU" || unit_class == "ALL"){
    ...
    if(length(AUCUPARAM)>0 && parameter_required("^TIMEU$", names(map_data)) && parameter_required("^CONCU$", names(map_data))){
      if(parameter_required(map_data$TIMEU, names(data_data)) && parameter_required(map_data$CONCU, names(data_data))){
        ...
        inputUnit8 <- toupper(c(as.character(auc_unit_tmp[1]), as.character(unique(data_data[, map_data$TIMEU])[[1]]), as.character(auc_unit_tmp[2])))
        outputUnit8 <- ifelse(!is.null(map_data$AUCOUTPUTUNIT), toupper(unlist(strsplit(unlist(strsplit(as.character(map_data$AUCOUTPUTUNIT), "/")), "[.]"))), NA)          
        ...
        if(testunit) {
            outputUnit8 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "AUMCU" || unit_class == "ALL"){
    ...
    if(length(AUMCUPARAM)>0 && parameter_required("^TIMEU$", names(map_data)) && parameter_required("^CONCU$", names(map_data))){
      if(parameter_required(map_data$TIMEU, names(data_data)) && parameter_required(map_data$CONCU, names(data_data))){
        ...
        inputUnit9 <- toupper(c(as.character(aumc_unit_tmp[1]), aumc_unit_tmp2, aumc_unit_tmp2, as.character(aumc_unit_tmp[2])))
        ...
        if(testunit) {
            outputUnit9 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "AUCNORMU" || unit_class == "ALL"){
    ...
    if(length(AUCNORMUPARAM)>0 && parameter_required("^TIMEU$", names(map_data)) && parameter_required("^CONCU$", names(map_data)) && parameter_required("^DOSE[0-9]*?U$", names(map))){
      if(parameter_required(map_data$TIMEU, names(data_data)) && parameter_required(map_data$CONCU, names(data_data)) && parameter_required("^DOSE(i{1}|[0-9]*?)U$", names(map))){
        ...
        inputUnit10 <- toupper(c(as.character(aucdn_unit_tmp[1]), aucdn_unit_tmp2, as.character(aucdn_unit_tmp[2]), aucdn_unit_tmp3))
        ...
        if(testunit) {
            outputUnit10 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "AURCU" || unit_class == "ALL"){
    ...
    if(length(AURCUPARAM)>0 && parameter_required("^CONCU$", names(map_data))){
      if(parameter_required(map_data$CONCU, names(data_data))){
        ...
        inputUnit11 <- toupper(c(as.character(aurc_unit_tmp[2]), as.character(aurc_unit_tmp[1]), as.character(aurc_unit_tmp[2])))
        ...
        if(testunit) {
            outputUnit11 <- toupper(as.character(map_data[[outputUnitLabel]]))
    ...
    if(length(CONCNORMUPARAM)>0 && parameter_required("^CONCU$", names(map_data)) && parameter_required("^DOSE(i{1}|[0-9]*?)U$", names(map))){
      if(parameter_required(map_data$CONCU, names(data_data)) && parameter_required("^DOSE(i{1}|[0-9]*?)U$", names(map))){
        if(parameter_required(map_data[as.numeric(parameter_indices("^DOSE(i{1}|[0-9]*?)U$", names(map)))], names(data_data))){
          ...
          inputUnit12 <- toupper(c(as.character(conc_unit_tmp[1]), as.character(conc_unit_tmp[2]), conc_unit_tmp2))
          ...
          if(testunit) {
              outputUnit12 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "RATEU" || unit_class == "ALL"){
    ...
    if(length(RATEUPARAM>0)) {
        ...
        inputUnit13 <- toupper(c(as.character(rate_unit_tmp[1]), as.character(unique(data_data[, map_data$TIMEU])[[1]])))
        ...
        if(testunit) {
            outputUnit13 <- toupper(as.character(map_data[[outputUnitLabel]]))
  ...
  if(unit_class == "VOLUMEWU" || unit_class == "ALL"){
    ...
    dose_by_weight <- FALSE
    if(parameter_required("^DOSELIST$", names(map_data)) && parameter_required("^DOSEULIST$", names(map_data))){
      doselist <- names(parameter_indices("^DOSELIST$", names(map_data), simplify=FALSE))
      dosevar <- ifelse(!is.null(doselist), ifelse(!is.null(map_data[,doselist]), unlist(strsplit(map_data[,doselist], ";")), NA), NA)
      doseulist <- names(parameter_indices("^DOSEULIST$", names(map_data), simplify=FALSE))
      doseuvar <- ifelse(!is.null(doseulist), ifelse(!is.null(map_data[,doselist]), unlist(strsplit(map_data[,doseulist], ";")), NA), NA)
    ...
    if(length(VOLUMEWUPARAM)>0 && parameter_required("^CONCU$", names(map_data)) && (parameter_required("^NORMBSU$", names(map_data)) || isTRUE(dose_by_weight))){
      if(parameter_required(map_data$CONCU, names(data_data)) && parameter_required(map_data$NORMBSU, names(data_data))){
        ...
        if(dose_by_weight){
          inputUnit14 <- toupper(c(as.character(vwu_unit_tmp[2]), as.character(vwu_dose_unit_tmp[2])))
        } else {
          inputUnit14 <- toupper(c(as.character(vwu_unit_tmp[2]), as.character(unique(data_data[, map_data$NORMBSU])[1])))
        }
        ...
        if(testunit) {
          outputUnit14 <- toupper(as.character(map_data[[outputUnitLabel]]))
        ...
        if(testunit){
          for(i in 1:2) {
            if(outputUnitFormat){
              ...
              } else if(i == 2 && inputUnit14[i] %in% dose_denominator && outputUnit14[i] %in% dose_denominator) {
                volumewUScaler[i] <- 1
                if(inputUnit14[i] != outputUnit14[i]){
                  warning("'", inputUnit14[i], "' to '", outputUnit14[i], "' VOLUMEWU conversion is not accounted for unit conversion")
                }
              } else if(i == 2 && ((inputUnit14[i] %in% dose_denominator && !(outputUnit14[i] %in% dose_denominator)) || (!(inputUnit14[i] %in% dose_denominator) && outputUnit14[i] %in% dose_denominator))) {
                volumewUScaler[i] <- 1
                valid_conversion <- FALSE
                warning("'", inputUnit14[i], "' to '", outputUnit14[i], "' VOLUMEWU conversion is not accounted for unit conversion")
              } else {
        ...
        if(testunit){
          ...
          if(!valid_input_unit || !valid_output_unit || !valid_conversion){
  ...
  if(unit_class == "CLWU" || unit_class == "ALL"){
    ...
    if(length(CLWUPARAM)>0 && parameter_required("^CONCU$", names(map_data)) && parameter_required("^TIMEU$", names(map_data)) && (parameter_required("^NORMBSU$", names(map_data)) || isTRUE(dose_by_weight))){
      if(parameter_required(map_data$CONCU, names(data_data)) && parameter_required(map_data$TIMEU, names(data_data)) && parameter_required(map_data$NORMBSU, names(data_data))){
        ...
        if(dose_by_weight){
          inputUnit15 <- toupper(c(as.character(clwu_unit_tmp[2]), as.character(unique(data_data[, map_data$TIMEU])[[1]]), as.character(clwu_dose_unit_tmp[2])))
        } else {
          inputUnit15 <- toupper(c(as.character(clwu_unit_tmp[2]), as.character(unique(data_data[, map_data$TIMEU])[[1]]), as.character(unique(data_data[, map_data$NORMBSU])[1])))
        }
        ...
        if(testunit) {
            outputUnit15 <- toupper(as.character(map_data[[outputUnitLabel]]))
            ...
            if(outputUnitFormat){
              ...
              if(inputMatch15[i] != missing_idx && outputMatch15[i] != missing_idx) {
                ...
                if(class[inputMatch15[i]] == c("V", "T", "M")[i] && class[outputMatch15[i]] == c("V", "T", "M")[i]) {
                  ...
                  clwUScaler[i] <- 1
                  warning(paste0("'CONCU' and/or '", map_data$TIMEU, " and/or 'NORMBSU' and/or '", outputUnitLabel, "' value provided via 'map' is not accounted for unit conversion"))
                }
              } else if(i == 3 && inputUnit15[i] %in% dose_denominator && outputUnit15[i] %in% dose_denominator) {
                clwUScaler[i] <- 1
                if(inputUnit15[i] != outputUnit15[i]){
                  warning("'", inputUnit15[i], "' to '", outputUnit15[i], "' CLWU conversion is not accounted for unit conversion")
                }
              } else if(i == 3 && ((inputUnit15[i] %in% dose_denominator && !(outputUnit15[i] %in% dose_denominator)) || (!(inputUnit15[i] %in% dose_denominator) && outputUnit15[i] %in% dose_denominator))) {
                clwUScaler[i] <- 1
                valid_conversion <- FALSE
                warning("'", inputUnit15[i], "' to '", outputUnit15[i], "' CLWU conversion is not accounted for unit conversion")
              } else {
                ...
                clwUScaler[i] <- 1
                warning(paste0("'CONCU' and/or '", map_data$TIMEU, "' and/or 'NORMBSU' and/or '", outputUnitLabel, "' value provided via 'map' is not valid for unit conversion"))
              }
            } else {
              valid_output_unit <- FALSE
            }
