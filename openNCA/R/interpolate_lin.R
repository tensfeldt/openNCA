...
### 2019-10-18/TGT/ change error messages from "auc_inerpolate_lin" to "interpolate_lin"
  ...
  #   stop("Error in interpolate_lin: 'time1' value is NULL or NA")
  # } else if(is.null(conc2) || is.na(conc2)) {
  #   stop("Error in interpolate_lin: 'conc2' value is NULL or NA")
  # } else if(is.null(time2) || is.na(time2)) {
  #   stop("Error in interpolate_lin: 'time2' value is NULL or NA")
  # } else if(is.null(est_time) || is.na(est_time)) {
  #   stop("Error in interpolate_lin: 'est_time' value is NULL or NA")
  # }
  # 
  # if(!is.numeric(conc1)) {
  #   stop("Error in interpolate_lin: 'conc1' value is not numeric")
  # } else if(!is.numeric(time1)) {
  #   stop("Error in interpolate_lin: 'time1' value is not numeric")
  # } else if(!is.numeric(conc2)) {
  #   stop("Error in interpolate_lin: 'conc2' value is not numeric")
  # } else if(!is.numeric(time2)) {
  #   stop("Error in interpolate_lin: 'time2' value is not numeric")
  # } else if(!is.numeric(est_time)) {
  #   stop("Error in interpolate_lin: 'est_time' value is not numeric")
  # }

  est_conc <- NA
  if(isTRUE(time1 < est_time) && isTRUE(est_time < time2)){
